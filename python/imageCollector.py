...
class ImageCollector:
    def collect(self, data_config):
        ...
        for category, info in categorys.items():
            ...
            # 画像ファイルの取得
            images = [f for f in os.listdir(self.temp_dir) if f.endswith(('.jpg', '.jpeg', '.png', '.bmp'))]
            if len(images) == 0:
            ...
            for img in train_imgs:
                shutil.move(os.path.join(self.temp_dir, img), os.path.join(train_dir, img))
            for img in val_imgs:
                shutil.move(os.path.join(self.temp_dir, img), os.path.join(eval_dir, img))
            print(f"学習用画像枚数: {len(train_imgs)} 検証用画像枚数: {len(val_imgs)}")
            # 一時保存フォルダ削除
            os.rmdir(self.temp_dir)

    # 指定枚数ダウンロードする
    def imageCrawl(self, keywords, target_num, save_dir, batch_size=100, max_retries=3):
        hash_set = set()
        unique_files = []
        total_saved = 0   # 総収集枚数
        # 一時保存フォルダ
        ...
        for keyword in keywords:
            ...
            # 最大リトライ回数に達したら終了
            # 総試行回数が1000超えたら終了
            while total_saved < target_num and retries < max_retries and try_times < 1000:
                ...
                for img_path  in unique_files:
                    shutil.move(os.path.join(temp_dir, img_path), os.path.join(save_dir, img_path))
    ...
    # 重複画像を削除
    def remove_duplicate(self, dir, hash_set, unique_files):
